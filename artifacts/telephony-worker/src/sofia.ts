...
export function gatewayXml(config: SipConfig): string {
  ...
  return `<?xml version="1.0" encoding="UTF-8"?>\n<include><gateway name="${xml(name)}">\n` +
    ...
    `<param name="username" value="${xml(token(config.extension, "extension"))}"/><param name="auth-username" value="${xml(token(config.authUsername, "auth username"))}"/><param name="password" value="${xml(config.password ?? "")}"/>` +
    ...
    `<param name="ping" value="${config.keepaliveIntervalSeconds ?? 30}"/>` +
